...
void vcan_sendccd(uint8 *ccdaddr, uint32 ccdsize) //vcan_sendccd((uint8 *)&CCD_BUFF[0],1*TSL1401_SIZE);
{
#define CMD_CCD     2
    ...
    uart_putbuff(FIRE_PORT, ccdaddr, ccdsize);      //再发送图像

    uart_putbuff(FIRE_PORT, cmdr, sizeof(cmdr));    //再发送命令
}


void Send_CCD(uint8 *CCDA_addr, uint32 CCDA_size, uint8 *CCDB_addr, uint32 CCDB_size) //vcan_sendccd((uint8 *)&CCD_BUFF[0],1*TSL1401_SIZE);
{
#define CMD_CCD     2
    uint8 cmdf[2] = {CMD_CCD, ~CMD_CCD};            //开头命令
    uint8 cmdr[2] = {~CMD_CCD, CMD_CCD};            //结尾命令
    ...
    uart_putbuff(FIRE_PORT, CCDA_addr, CCDA_size);      //再发送图像
    uart_putbuff(FIRE_PORT, CCDB_addr, CCDB_size);      //再发送图像
...
#define CMD_WARE     3
    ...
    uart_putbuff(UART3, cmdf, sizeof(cmdf));    //先发送前命令
    uart_putbuff(UART3, wareaddr, waresize);    //发送数据
    uart_putbuff(UART3, cmdr, sizeof(cmdr));    //发送后命令
