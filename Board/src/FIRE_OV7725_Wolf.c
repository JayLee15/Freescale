...
uint8   *v7725_wolf_img_buff;

volatile IMG_STATUS_e      v7725_wolf_img_flag = IMG_FINISH;   //图像状态
...
void ov7725_wolf_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTB_B0_IN, (void *)v7725_wolf_img_buff, PTA27, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
...
void ov7725_wolf_vsync(void)
{
    ...
    //场中断需要判断是场结束还是场开始
    if(v7725_wolf_img_flag == IMG_START)                   //需要开始采集图像
    {
        v7725_wolf_img_flag = IMG_GATHER;                  //标记图像采集中
        disable_irq(PORTA_IRQn);

        DMA_EN(CAMERA_DMA_CH);                  //使能通道CHn 硬件请求
        DMA_DADDR(CAMERA_DMA_CH) = (uint32)v7725_wolf_img_buff;    //恢复地址
    }
    else                                        //图像采集错误
    {
        disable_irq(PORTA_IRQn);                        //关闭PTA的中断
        v7725_wolf_img_flag = IMG_FAIL;                    //标记图像采集失败
    }
}
...
{
    v7725_wolf_img_flag = IMG_FINISH ;
...
{
    v7725_wolf_img_flag = IMG_START;                    //开始采集图像
    PORTA_ISFR = ~0;                                    //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
    enable_irq(PORTA_IRQn);                             //允许PTA的中断
    while(v7725_wolf_img_flag != IMG_FINISH)            //等待图像采集完毕
    {
        if(v7725_wolf_img_flag == IMG_FAIL)             //假如图像采集错误，则重新开始采集
        {
            v7725_wolf_img_flag = IMG_START;            //开始采集图像
            PORTA_ISFR = ~0;                            //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
            enable_irq(PORTA_IRQn);                     //允许PTA的中断
