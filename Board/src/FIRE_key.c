 ...
 * 定义 KEY 编号对应的管脚
 */
PTXn_e KEY_PTxn[KEY_MAX] = {PTC7, PTC8, PTC10, PTC11,PTC12,PTC13};


void D_KEY_init(void)
{
     port_init (PTA17, PF | ALT1 | PULLUP );
     port_init (PTA16, PF | ALT1 | PULLUP );
     port_init (PTA15, PF | ALT1 | PULLUP );
     port_init (PTA14, PF | ALT1 | PULLUP );
}

KEY_STATUS_e D_KEY_get(PTXn_e key)
{
    if(gpio_get( key) == KEY_DOWN)
    {
        return KEY_DOWN;
    }
    return KEY_UP;
