...
{
    gpio_init (LCD_BL, GPO, 0); //LCD背光管脚为0，表示点亮LCD
...
void LCD_ILI9341_ptlon(Site_t site, Size_t size)
{
    ...
    LCD_ILI9341_WR_DATA(site.x - ((site.x >> 8) << 8));
    LCD_ILI9341_WR_DATA((site.x + size.W - 1) >> 8); //end
    LCD_ILI9341_WR_DATA((site.x + size.W - 1) - (((site.x + size.W - 1) >> 8) << 8));

    LCD_ILI9341_WR_CMD(0X2B);
    LCD_ILI9341_WR_DATA(site.y >> 8); //start
    LCD_ILI9341_WR_DATA(site.y - ((site.y >> 8) << 8));
    LCD_ILI9341_WR_DATA((site.y + size.H - 1) >> 8); //end
    LCD_ILI9341_WR_DATA((site.y + size.H - 1) - (((site.y + size.H - 1) >> 8) << 8));
